...
export const POST: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		const securePayload: QueryPayload = {
			...
			where: clientPayload.where ? { op: LogicalOperator.AND, conditions: [supplierIdCondition, clientPayload.where] } : { op: LogicalOperator.AND, conditions: [supplierIdCondition] },
			limit: 1
		};
