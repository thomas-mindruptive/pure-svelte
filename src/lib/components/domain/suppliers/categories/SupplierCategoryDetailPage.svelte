...
<script lang="ts">
	...
	$effect(() => {
		...
		const processPromises = async () => {
			...
			try {
				const [assignmentDetails, offerings] = await Promise.all([
				...
				if (!validationResult.success) {
					...
					throw new Error(`SupplierCategoryDetailPage: Received invalid data structure from the API: ${JSON.stringify(validationResult.error.issues)}`);
				}
...
{:else}
	<div class="detail-page-layout">
		...
		<div class="grid-section">
			<button class="pc-grid__createbtn" onclick={handleOfferingCreate}>Create Offering</button>
			<OfferingGrid
...
<style>
	h1 { margin: 0; }
	p { margin: 0.5rem 0 0 0; color: var(--color-muted); max-width: 80ch; }
	.comment {
		font-style: italic;
		margin-top: 0.75rem;
	...
	.grid-section { background: var(--color-background); border-radius: 8px; border: 1px solid var(--color-border); }
</style>