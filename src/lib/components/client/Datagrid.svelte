<script lang="ts">
  ...
  type ID = string | number; // Universal ID type for row identification
  ...
  export type DryRunResult = {
    ...
    [k: string]: any;                       // Extensible for domain-specific data
  };
  ...
  export type ConfirmContext<T = any> = {
    ...
    dryRun?: DryRunResult;  // Results from dry-run analysis
  ...
  export type ConfirmResult = {
    ...
    options?: Record<string, any>;      // Additional options (e.g., {cascade: true})
  ...
  export type DeleteStrategy<T = any> = {
    ...
    execute: (ids: ID[], options?: Record<string, any>) => Promise<void>; // Required: perform the actual deletion
  };
