...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    const body = (await request.json()) as AssignmentRequest<WholesalerItemOffering, Attribute, Omit<WholesalerOfferingAttribute, "id">>;
    const { parent1Id: offeringId, parent2Id: attributeId, data: wholesalerOfferingAttribute } = body;
    ...
    const checkResult = await db.request().input("offeringId", offeringId).input("attributeId", attributeId).query(`
                SELECT 
    ...
    const result = await db
      ...
      .input("value", wholesalerOfferingAttribute.value || null).query(`
...
export const PUT: RequestHandler = async ({ request }) => {
  ...
  try {
    const body = (await request.json()) as AssignmentUpdateRequest<WholesalerItemOffering, Attribute, WholesalerOfferingAttribute>;
    const { parent1Id: offeringId, parent2Id: attributeId, data: offeringAttribute } = body;
    ...
    const existsCheck = await db.request().input("offeringId", offeringId).input("attributeId", attributeId).query(`
                SELECT 
                    woa.offering_id, 
                    woa.attribute_id,
                    woa.value,
    ...
    const result = await db
      ...
      .input("offeringId", offeringId)
      .input("attributeId", attributeId)
      .input("value", offeringAttribute.value).query(`
...
export const DELETE: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    const { parent1Id: offeringId, parent2Id: attributeId, cascade = false } = body;
    ...
    const assignmentResult = await db.request().input("offeringId", offeringId).input("attributeId", attributeId).query(`
                SELECT 
                    woa.offering_id, 
                    woa.attribute_id,
                    a.name as attribute_name,
    ...
    const deleteResult = await db.request().input("offeringId", offeringId).input("attributeId", attributeId).query(`
