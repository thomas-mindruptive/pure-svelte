...
export interface QueryConfig {
  ...
  joinConfigurations?: { [viewName: string]: { from: string; joins: JoinClause[]; exampleQuery?: QueryPayload } };
}
...
export const supplierQueryConfig: QueryConfig = {
  ...
  allowedTables: {
    ...
    'dbo.wholesaler_categories': [
      ...
      // For joins to categories and offerings 
      // From wholesalers (w)
      'w.wholesaler_id', 'w.name', 'w.region', 'w.status', 'w.dropship', 'w.website', 'w.created_at', 'w.b2b_notes',

      // From wholesaler_categories (wc)
      'wc.wholesaler_id', 'wc.category_id', 'wc.comment', 'wc.link', 'wc.created_at',

      // From product_categories (pc)
      'pc.category_id', 'pc.name', 'pc.description',

      // From subquery (oc)
      'oc.offering_count',

      // Aliased versions - common patterns
  ...
  joinConfigurations: {
    ...
    'supplier_categories': {
      ...
      joins: [
        ...
        // NEU: LEFT JOIN für offering counts
        {
          type: JoinType.LEFT,
          table: '(SELECT wholesaler_id, category_id, COUNT(*) as offering_count FROM dbo.wholesaler_item_offerings GROUP BY wholesaler_id, category_id)',
          alias: 'oc',
          on: 'wc.wholesaler_id = oc.wholesaler_id AND wc.category_id = oc.category_id'
        }
      ],
      exampleQuery: {
        select: [
