...
<script lang="ts">
		...
		const processPromises = async () => {
			...
			try {
				...
				const validationResult =
					OfferingDetailLinks_LoadDataSchema.safeParse(
						dataToValidate,
					);

				if (!validationResult.success) {
					log.error(
						"(OfferDetailLinksPage) Zod validation failed",
				...
				const message =
					rawError.message ||
					"Failed to load or validate link details.";
	...
	async function handleAssignLink(event: SubmitEvent) {
		...
		if (!resolvedData || !resolvedData.offering) {
			addNotification(
				"An offering must be saved before assigning links.",
				"error",
			);
			return;
		}
		...
		<div class="grid-section">
			<div class="assignment-section">
				...
				{#if !resolvedData.offering}
					<p class="field-hint">
						You must save the new offering first before you can add
						links.
					</p>
				{/if}
			...
			{#if !resolvedData.offering}
				<p class="field-hint">
					You must save the new offering first before you can add/see
					links.
				</p>
			{:else}
				<LinkGrid
