...
function buildOnClause(on: JoinConditionGroup, ctx: BuildContext): string {
  const conditions = on.conditions
    .map((cond) => {
 ...
 */
export function buildQuery<T>(payload: Partial<QueryPayload<T>> | undefined, config: QueryConfig, namedQuery?: string, fixedFrom?: FromClause) {
  // Declare query component variables upfront to avoid redeclaration in conditional branches.
  // These will be populated from either the named query config or the dynamic payload.
  let select: QueryPayload<T>["select"];
  let joins: JoinClause[] | undefined;
  let where: WhereCondition<T> | WhereConditionGroup<T> | undefined;
  let orderBy: SortDescriptor<T>[] | undefined;
  let limit: number | undefined;
  let offset: number | undefined;
  let realJoins: JoinClause[] = [];
  ...
  // Two execution paths: Named queries (predefined JOINs) vs Dynamic queries (ad-hoc).

  if (namedQuery && config.predefinedQueryies) {
    // === PATH 1: NAMED QUERY (PREDEFINED JOIN) ===
    // Named queries use server-side JOIN configurations for maximum security.
    // The predefined config provides table structure, JOINs, and default values.
    const predefQueriesConfig = config.predefinedQueryies[namedQuery as keyof typeof config.predefinedQueryies];
    if (!predefQueriesConfig) throw new Error(`Named query '${namedQuery}' is not defined.`);

    let mergedQueryPayload: QueryPayload<T>;
    if (payload) {
      // Merge strategy: Predefined config provides defaults, client payload overrides.
      // Order matters: { ...defaults, ...overrides } ensures client values take precedence.
      mergedQueryPayload = { ...predefQueriesConfig, ...payload } as QueryPayload<T>;
    } else {
      // No client payload: use predefined config entirely.
      // Requires that config includes both 'select' and 'from' for a valid query.
	  if (!predefQueriesConfig.select || !predefQueriesConfig.from) {
		throw new Error(`If no payload is passed, "from" and "select" must be in query config "${namedQuery}"`);
	  }
      mergedQueryPayload = predefQueriesConfig as QueryPayload<T>;
    }

    // Extract query components from the merged payload.
    ({ select, joins, where, orderBy, limit, offset } = mergedQueryPayload);

    fromClause = `${predefQueriesConfig.from.table} ${predefQueriesConfig.from.alias} `;
    fromTableForMetadata = namedQuery;

    // Use JOINs from merged payload (already contains predefined JOINs from config due to spread)
    realJoins = joins || [];
  } else {
    // === PATH 2: DYNAMIC QUERY (AD-HOC) ===
    // Client provides full query structure. Validate against branded schemas at runtime.
    if (!payload) {
      throw new Error(`Payload must be defined if it is not a named query. `);
    }

    // Validate required fields for dynamic queries
    if (!payload.select || !Array.isArray(payload.select) || payload.select.length === 0) {
      throw new Error("Dynamic query must have a 'select' clause with at least one column.");
    }

    // Extract query components directly from the client payload.
    ({ select, joins, where, orderBy, limit, offset } = payload as QueryPayload<T>);
