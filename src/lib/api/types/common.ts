...
/**
 * @file Common API Types - FINAL & COMPLETE
...
export interface AssignmentRequest<TParentId, TChildId> {
	parentId: TParentId; childId: TChildId; comment?: string; link?: string;

}
export interface AssignmentSuccessData<TAssignment> extends Record<string, unknown> {
	assignment: TAssignment; meta: {
		assigned_at: string; parent_name: string; child_name: string;
...
export interface AssignmentConflictResponse<TDetails extends Record<string, unknown>> extends ApiErrorResponse {
	error_code: 'ASSIGNMENT_CONFLICT'; details: TDetails;

}
export type AssignmentApiResponse<TAssignment, TConflictDetails extends Record<string, unknown>> = AssignmentSuccessResponse<TAssignment> | AssignmentConflictResponse<TConflictDetails> | ApiErrorResponse;

// --- Delete Responses ---
export interface RemoveAssignmentRequest<TParentId, TChildId> {
	parentId: TParentId; childId: TChildId; cascade?: boolean;

}
export interface DeleteSuccessData<TDeletedResource> extends Record<string, unknown> {
	deleted_resource: TDeletedResource; cascade_performed: boolean; dependencies_cleared: number;
...
export interface DeleteConflictResponse<TDependencies> extends ApiErrorResponse {
	error_code: 'DEPENDENCY_CONFLICT'; dependencies: TDependencies; cascade_available: boolean;

}
export type DeleteApiResponse<TDeletedResource, TDependencies> = DeleteSuccessResponse<TDeletedResource> | DeleteConflictResponse<TDependencies> | ApiErrorResponse;
