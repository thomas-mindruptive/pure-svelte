...
describe('parseDimensions', () => {
  ...
  describe('Range of multi-dimensional format (NEW)', () => {
    ...
    it('should reject invalid range "5x6cm - 2x3cm" (min > max)', () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid) expect(result.error).toContain('Minimum must be less than or equal to maximum');
    });
  });
  ...
  describe('Error cases', () => {
    it('should reject missing unit "3"', () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid) expect(result.error).toContain('Invalid dimension format');
    });
    ...
    it('should reject invalid range "5-3cm" (min > max)', () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid) expect(result.error).toContain('Minimum dimension must be less than or equal to maximum');
    });
    ...
    it('should reject invalid bracket content "[abc]"', () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid) expect(result.error).toContain('Invalid bracket content');
    });
