...
# Status from last chat session


Erweiterte Konzeptvergleich: Alt vs. Neu (komplett)
ALTES KONZEPT: Monolithische Pages
routes/suppliers/+page.svelte              # Inline <table>, Inline Forms
routes/suppliers/[id]/+page.svelte         # Inline <table>, Inline Forms
Charakteristika:

UI: Inline HTML-Tables, Inline Form-Code
Logic: Direkte use:enhance pro Form
Navigation: Separate Pages
Code: Viel Duplikation


NEUES KONZEPT: Modulare Komponenten + Single Page
routes/supplierbrowser/+page.svelte        # Orchestriert Komponenten
lib/components/grids/SupplierGrid.svelte   # Wiederverwendbare Grids
lib/components/forms/SupplierForm.svelte   # Wiederverwendbare Forms
lib/utils/formEnhance.ts                   # Generisches Enhance-System

DETAILLIERTE ÄNDERUNGEN:
1. GRID-SYSTEM
ALT: Inline HTML-Tables
svelte<!-- In jeder Page -->
<table>
  ...
  <tbody>
    {#each suppliers as supplier}
      <tr>
        ...
        <td><button on:click={() => deleteSupplier(supplier)}>Delete</button></td>
      </tr>
    {/each}
  </tbody>
</table>
NEU: Generisches Datagrid + Entity-Grids
svelte<!-- Basis: Generisches Datagrid -->
<Datagrid rows={data} columns={cols} onsort={...} ondelete={...}>
  <div slot="cell" let:row let:column>{value}</div>
</Datagrid>

<!-- Wrapper: Entity-spezifische Grids -->
<SupplierGrid suppliers={data} onrowclick={...} ondelete={...} />
<CategoryGrid categories={data} onrowclick={...} ondelete={...} />
Vorteile NEU:

Ein Datagrid.svelte für alle Entities
Konsistente Sortierung, Pagination, Delete-Logic
Entity-Grids kapseln spezifische Business-Rules


2. FORM-SYSTEM
ALT: Inline Forms mit Duplikation
svelte<!-- In jeder Page -->
<form method="POST" action="?/update" use:enhance={() => {
  return async ({ result }) => {
    if (result.type === "failure") {
      addNotification(result.data?.error, "error");
    }
  ...
  <input name="name" bind:value={supplier.name} />
  <input name="region" bind:value={supplier.region} />
  <button type="submit">Save</button>
</form>
NEU: Wiederverwendbare Forms + Generisches Enhance
svelte<!-- Generisches Enhance-System -->
// lib/utils/formEnhance.ts
export const supplierEnhance = () => createGenericEnhance({
  entityType: 'wholesaler',
  createdMessage: 'Supplier created!',
  updatedMessage: 'Supplier updated!',
...
Ein Enhance-System für alle Entities
Forms als wiederverwendbare Komponenten
Konsistente Success/Error-Behandlung


3. QUERY-SYSTEM
ALT: Hardcoded SQL in +page.server.ts
typescript// Duplikation in jeder Route
const suppliers = await db.query`SELECT * FROM wholesalers WHERE ...`;
const categories = await db.query`SELECT * FROM categories WHERE ...`;
NEU: Generischer QueryBuilder + Domain-Config
typescript// Generischer Builder
...
// Domain-spezifische Config
export const supplierQueryConfig = {
  allowedTables: { 'dbo.wholesalers': ['name', 'region', ...] },
  joinConfigurations: { 'supplier_categories': {...} }
};
Vorteile NEU:

Typsichere Query-API
Security durch Whitelisting
JOIN-Support für komplexe Queries


4. NAVIGATION & STATE
ALT: Server-Side Navigation
/suppliers → /suppliers/123 → /suppliers/123/categories/456
NEU: Client-Side Navigation mit URL-State
/supplierbrowser?supplier=123&level=categories&category=456
Master-Detail Layout:

Oben: Parent-Form (z.B. Supplier-Edit)
Mitte: Children-Grid (z.B. Categories)
Unten: Create-Form (z.B. New Category)


KERNIDEE: Modulare Wiederverwendbarkeit
ALT: Jede Page schreibt alles neu (Tables, Forms, Enhance-Logic)
NEU: Baukasten-System mit wiederverwendbaren Komponenten

1x Datagrid → für alle Entity-Listen
1x Enhance-System → für alle Form-Actions
1x QueryBuilder → für alle Datenbank-Queries
Nx Entity-Komponenten → dünne Wrapper um generische Basis

Ergebnis: Weniger Code, mehr Konsistenz, bessere Wartbarkeit
