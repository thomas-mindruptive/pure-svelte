...
<script lang="ts">
	...
	// NEW: Import the async and resolved types for this page.
	import {
	...
	$effect(() => {
		...
		const processPromises = async () => {
			...
			try {
				...
				const dataToValidate = { supplier, assignedCategories, availableCategories };
				...
				if (!validationResult.success) {
					log.error('(SupplierDetailPage) Zod validation failed', validationResult.error.issues);
				...
				log.error('(SupplierDetailPage) Promise processing failed', { rawError });
			} finally {
				if (!aborted) {
	 ...
	 * Calls the API to update the supplier.
	 */
	async function handleSupplierUpdate(event: { data: Record<string, any> }) {
		...
		try {
			...
			// Update local data to avoid a full page reload.
			resolvedData.supplier = updatedSupplier;
		} catch (error) {
